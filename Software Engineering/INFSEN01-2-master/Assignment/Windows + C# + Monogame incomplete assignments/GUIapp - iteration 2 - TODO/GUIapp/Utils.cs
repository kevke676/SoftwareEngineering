...
namespace GUIapp
{
    ...
    public class List<T> : Iterator<T>
    {
        ...
        {
            if (itemAmount >= arrayLength)
            {
                arrayLength *= 2;
                T[] biggerArray = new T[arrayLength];
                Array.Copy(TArray, biggerArray, itemAmount);
            }
            else
            {
                TArray[itemAmount] = item;
            }
            itemAmount++;
        }
        ...
        public Option<T> GetNext()
        {
            ...
            return new Some<T>(TArray[position]);
        }
    ...
    public class DefaultUpdateVisitor : UpdateVisitor
    {
        ...
        public void UpdateGui(GuiManager gui_manager, float dt)
        {
            ...
            while (gui_manager.elements.GetNext().Visit(() => false, _ => true))
            {
                gui_manager.elements.GetCurrent().Visit(() => { }, i => { i.Update(this, dt); });
            }
        }
    }
